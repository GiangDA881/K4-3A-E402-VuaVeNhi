...
|---|---|---|---|
| Nguyễn Xuân Trường Giang | 2A202602446 | Đội trưởng (Team Lead) & Prompt Architect | Định hình Canvas 4 ô, chốt bài toán & lát cắt sản phẩm, chủ trì soạn thảo `spec.md` (§1-§4), thiết kế System Prompt, quản trị repo và chịu trách nhiệm nộp đủ 5 checkpoint (CP1-CP5). |
| Võ Doanh Nhân | 2A202602770 | Tool & Backend Engineer | Xây dựng pipeline kết nối mô hình LLM API, chuẩn hóa schemas & tools/function calling, xử lý các trường hợp biên và kịch bản rủi ro kỹ thuật (§5-§6 trong `spec.md`). |
| Nguyễn Nhân Sâm | 2A202602672 | Eval & Red-Team QA Lead | Xây dựng bộ test cases Golden Set trong `eval/`, kiểm thử adversarial attacks, chạy benchmark đo lường số đo CP3 & CP4 và phân tích nguyên nhân lỗi. |
| Đào Đức Hải | 2A202602752 | UI/UX & Validation Coordinator | Thiết kế sơ đồ luồng hoạt động (CP2), xây dựng giao diện Streamlit/Web UI trong `codebase/`, quay video thao tác 30s (CP3) & video pitch dự phòng (CP5), điều phối khảo sát & validation ngoài nhóm (R6). |
